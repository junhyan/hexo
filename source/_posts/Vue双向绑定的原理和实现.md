...
date: 2018-05-28
---
偶然间看到一篇VUE双向绑定的实现原理的文章，写的很好，比直接去无脑的看VUE源码要明了很多，所以转到自己的博客中，为了方便之后的阅读和分享给更多的人。文章转自http://www.cnblogs.com/libin-1/p/6893712.html
## vue数据双向绑定原理
...
console.log(Book.name);  // 《vue权威指南》
```
我们通过Object.defineProperty( )设置了对象Book的name属性，对其get和set进行重写操作，顾名思义，get就是在读取name属性这个值触发的函数，set就是在设置name属性这个值触发的函数，所以当执行 Book.name = 'vue权威指南' 这个语句时，控制台会打印出 "你取了一个书名叫做vue权威指南"，紧接着，当读取这个属性时，就会输出 "《vue权威指南》"，因为我们在get函数里面对该值做了加工了。如果这个时候我们执console.log(Book);，控制台会输出什么？
结果：
![](Vue双向绑定的原理和实现/result_demo.png)

## 思路分析
实现mvvm主要包含两个方面，数据变化更新视图，视图变化更新数据
![](Vue双向绑定的原理和实现/mvvm.jpg)
